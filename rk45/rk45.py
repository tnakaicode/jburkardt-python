...
#
  ...
  plt.xlabel ( '<-- T -->' )
  plt.ylabel ( '<-- Y(T) -->' )
  label = 'Predator rk5 solution with dt = ' + str ( dt )
  plt.title ( label )
...
# 
  ...
  for i in range ( 0, n ):
    ...
    k6 = dt * ( yprime ( t[i] + c[5] * dt, y[i,:] + a[5,0] * k1 \
      + a[5,1] * k2 + a[5,2] * k3 + a[5,3] * k4 + a[5,4] * k5 ) )
...
#
  ...
  for i in range ( 0, n ):
    ...
    k5 = dt * yprime ( t[i] + c[4] * dt, y[i,:] + a[4,0] * k1 \
      + a[4,1] * k2 + a[4,2] * k3 + a[4,3] * k4 )
    k6 = dt * yprime ( t[i] + c[5] * dt, y[i,:] + a[4,0] * k1 \
      + a[5,1] * k2 + a[5,2] * k3 + a[5,3] * k4 + a[5,4] * k5 )
