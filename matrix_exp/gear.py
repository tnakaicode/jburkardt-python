...
#
  ...
  if ( 0 < js and 0 < ks ):
    ...
    for p in range ( 1, phi + 1 ):
      alpha[w] = 2 * p * np.pi / float ( 2 * n + 2 - j - k )
      w = w + 1
 
    phi = ( ( j - 1 ) // 2 )
    for p in range ( 1, phi + 1 ):
      alpha[w] = 2 * p * np.pi / float ( j )
      w = w + 1
 
    phi = ( ( k - 1 ) // 2 )
    for p in range ( 1, phi + 1 ):
      alpha[w] = 2 * p * np.pi / float ( k )
      w = w + 1
    ...
    phi = n + 1 - ( ( j + k + 1 ) // 2 )
    for p in range ( 1, phi + 1 ):
      alpha[w] = ( 2 * p - 1 ) * np.pi / float ( 2 * n + 2 - j - k )
      w = w + 1
...
if ( __name__ == '__main__' ):
  ...
  timestamp ( )
  gear_test ( )
  gear_determinant_test ( )
  gear_eigenvalues_test ( )
  timestamp ( )
