...
#
    ...
    for l in range(1, nb + 1):
        ...
        for k in range(1, nwall[l - 1] + 1):
            arcl = arcl + abs(wall[k - 1, l - 1] -
                              wall[(k % nwall[l - 1]), l - 1])
#
#  Compute core radius.
#
    r0 = 0.5 * arcl / matdim
...
#
    ...
    for l in range(1, nb + 1):
        ...
        for k in range(1, nwall[l - 1] + 1):
            zz = wall[(k + nwall[l - 1] - 2) % nwall[l - 1],
                      l - 1] - wall[(k % nwall[l - 1]), l - 1]
...
#
        ...
        for k in range(1, nwall[l - 1] + 1):
            ...
            value2 = (zcr[kp - 1, l - 1] - zcr[k - 1, l - 1]) * \
                value1.conjugate()
        ...
        for l2 in range(1, nb + 1):
            ...
            for j in range(1, nwall[l2 - 1] + 1):
                ...
                for i in range(2, nwall[l1 - 1] + 1):
                    zi = np.exp(
                        (wall[i - 1, l1 - 1] - zcr[j - 1, l2 - 1]) * pidp)
                    zz = zi - 1.0 / zi
    ...
    for i in range(1, matdim + 1):
        ...
        for j in range(i + 1, matdim + 1):
            ...
            for k in range(i + 1, matdim + 1):
                rmatrx[j - 1, k - 1] = rmatrx[j - 1, k - 1] - \
                    rmatrx[j - 1, i - 1] * rmatrx[i - 1, k - 1]
