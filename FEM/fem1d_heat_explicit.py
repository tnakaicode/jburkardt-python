...
#
                for j in range(0, 2):
                    ...
                    bj, dbjdx = basis_function(
                        basis, element, x, element_q[quad])
...
#
        for quad in range(0, quad_num):
            ...
            for i in range(0, 2):
                ...
                bi, dbidx = basis_function(
                    test, element, node_x, element_q[quad])
#
#  Consider the BASIS-th basis function, which is used to form the
#  value of the solution function.
#
                for j in range(0, 2):
                    ...
                    bj, dbjdx = basis_function(
                        basis, element, node_x, element_q[quad])
...
def basis_function(index, element, node_x, point_x):
    ...
    if (index == element):
        b = (node_x[element + 1] - point_x) / \
            (node_x[element + 1] - node_x[element])
        dbdx = - 1.0 / (node_x[element + 1] - node_x[element])
    elif (index == element + 1):
        b = (point_x - node_x[element]) / \
            (node_x[element + 1] - node_x[element])
        dbdx = + 1.0 / (node_x[element + 1] - node_x[element])
