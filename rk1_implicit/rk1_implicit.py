...
#
  ...
  print ( '' )
  print ( 'rk1_implicit_test():' )
  print ( '  python version: ' + platform.python_version ( ) )
  print ( '  numpy version:  ' + np.version.version )
  print ( '  Test rk1_implicit.' )

  tspan = np.array ( [ 0.0, 2.0 ] )
  y0 = 5.1765
  n = 100
  humps_rk1_implicit ( tspan, y0, n )

  tspan = np.array ( [ 0.0, 5.0 ] )
  y0 = np.array ( [ 5000, 100 ] )
  n = 200
  predator_prey_rk1_implicit ( tspan, y0, n )
#
#  Terminate.
#
  print ( '' )
...
def rk1_implicit ( f, tspan, y0, n ):

#*****************************************************************************80
#
## rk1_implicit() uses Runge-Kutta order 1 implicit method to solve an ODE.
#
#  Discussion:
#
...
#
  ...
  for i in range ( 0, n ):
    ...
    yp = fsolve ( rk1_implicit_residual, yp, args = ( f, to, yo, tp ) )
...
def rk1_implicit_residual ( yp, f, to, yo, tp ):

#*****************************************************************************80
#
## rk1_implicit_residual() evaluates the rk1 implicit residual.
#
#  Discussion:
#
...
def humps_rk1_implicit ( tspan, y0, n ):

#*****************************************************************************80
#
## humps_rk1_implicit(): humps ODE using rk1_implicit().
#
#  Licensing:
#
#    This code is distributed under the MIT license.
...
#
  ...
  print ( '' )
  print ( 'humps_rk1_implicit():' )
  print ( '  Solve the humps ODE system using rk1_implicit().' )

  t, y = rk1_implicit ( humps_deriv, tspan, y0, n )
  ...
  plt.xlabel ( '<--- T --->' )
  plt.ylabel ( '<--- Y(T) --->' )
  plt.title ( 'humps_ode() solved by rk1_implicit()' )

  filename = 'humps_rk1_implicit.png'
...
def predator_prey_rk1_implicit ( tspan, y0, n ):

#*****************************************************************************80
#
## predator_prey_rk1_implicit(): predator ODE using rk1_implicit().
#
#  Licensing:
#
#    This code is distributed under the MIT license.
...
#
  ...
  print ( '' )
  print ( 'predator_prey_rk1_implicit():' )
  print ( '  Solve the predator prey ODE system using rk1_implicit().' )

  t, y = rk1_implicit ( predator_prey_deriv, tspan, y0, n )
  ...
  plt.title ( 'predator prey ODE solved by rk1_implicit()' )

  filename = 'predator_prey_rk1_implicit.png'
  plt.savefig ( filename )
  print ( '  Graphics saved as "%s"' % ( filename ) )
  plt.show ( block = False )
  plt.close ( )
