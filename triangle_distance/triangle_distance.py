...
#
  ...
  triangle_distance_histogram_test ( rng )

  triangle_distance_stats_test ( rng )

  triangle_equilateral_distance_pdf_test ( rng )

  triangle_right_distance_pdf_test ( rng )

  triangle_right_error_test ( rng )

  triangle_sample_test ( rng )

  triangle_sides_test ( rng )
#
#  Terminate.
#
  print ( '' )
...
#  Input:
#
#    integer INDX, the index of the function.
#
#    real T, the arguments.
#
#    real A, B, the length of the two legs of the triangle.  A <= B.
#
#    real C, the length of the hypotenuse.
#
#    real ALPHA, the angle BAC, in radians.
#
#    real H, the value of A * cos ( ALPHA ).
#
#  Output:
#
...
#
  if ( indx == 1 ):
    ...
    print ( '  Called with INDX = ', indx, ', but only values 1:4 are legal.' )
    return None
...
#  Input:
#
#    real D(*), the distance between two points in the triangle.
#
#    real A, B, the length of the two legs of the triangle.  A <= B.
#
#    real C, the length of the hypotenuse.
...
#
  ...
  if ( indx == 1 ):
    ...
    print ( '  Called with INDX = ', indx, ', but only values 1:2 are legal.' )
    return None
...
#  Input:
#
#    integer INDX, the index of the desired theta function.
#    1 <= INDX <= 6.
#
#    real A, B, the length of the two legs of the triangle.  A <= B.
#
...
#
  ...
  if ( indx == 1 ):
    value = ( a / b ) * ( 2.0 * alpha + np.pi ) + ( 2.0 * b / a ) * ( np.pi - alpha ) + 6.0
  elif ( indx == 2 ):
    ...
    print ( '  Called with INDX = ', indx, ', but only values 1:6 are legal.' )
    return None
...
def triangle_distance_stats ( v, n, rng ):

#*****************************************************************************80
#
## triangle_distance_stats() estimates distance statistics for a given triangle.
#
#  Licensing:
#
#    This code is distributed under the MIT license.
...
#
  ...
  p = triangle_sample ( v, n, rng )
  q = triangle_sample ( v, n, rng )
...
def triangle_distance_stats_test ( rng ):
...
#
  ...
  mu, var = triangle_distance_stats ( v, n, rng )
...
def triangle_distance_histogram ( v, n, filename, rng ):
...
def triangle_distance_histogram_test ( rng ):
...
#
  ...
  triangle_distance_histogram ( v, n, filename, rng )
#
#  Unit right triangle.
#
  v = np.array ( [  \
...
#  Output:
#
#    real PDF, the probability density function for that distance.
#
  import numpy as np
#
...
def triangle_equilateral_distance_pdf_test ( rng ):
...
#  Input:
#
#    real V(3,2), the X and Y coordinates of the vertices.
#
#    string LABEL, a title for the triangle.
#
  if ( 0 < len ( label ) ):
    print ( '' )
    print ( label )
...
#  Input:
#
#    real V(3,2), the vertices of the right triangle.
#
#    real D, the distance.
#
#  Output:
...
def triangle_right_distance_pdf_test ( rng ):
...
#  Output:
#
#    real ERR, the value of a^2+b^2-c^2.
#
  import numpy as np
#
...
def triangle_right_error_test ( rng ):
...
#
  ...
  print ( '  triangle_right_error() reports right triangle error a^2+b^2-c^2' )
...
#
  ...
  print ( '  triangle_sample() uniformly samples a triangle.' )
...
#  Output:
#
#    real A, B, C, the triangle side lengths.
#    with A <= B <= C.
#
  import numpy as np
#
...
#
  ...
  print ( '  triangle_sides() reports the lengths of the sides of a triangle.' )
