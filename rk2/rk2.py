...
#
  ...
  print ( '  Test rk2() for solving an ordinary differential equation.' )

  tspan = np.array ( [ 0.0, 2.0 ] )
  y0 = 5.1765
  n = 100
  rk2_humps_test ( tspan, y0, n )

  tspan = np.array ( [ 0.0, 5.0 ] )
  y0 = np.array ( [ 5000, 100 ] )
  n = 200
  rk2_predator_prey_test ( tspan, y0, n )

  tspan = np.array ( [ 0.0, 1.0 ] )
  y0 = np.array ( [ 0.0 ] )
  n = 27
  rk2_stiff_test ( tspan, y0, n )
#
#  Terminate.
#
  print ( '' )
...
def rk2_humps_test ( tspan, y0, n ):

#*****************************************************************************80
#
## rk2_humps_test() solves humps_ode() using rk2().
#
#  Licensing:
#
#    This code is distributed under the MIT license.
...
#
  ...
  print ( '' )
  print ( 'rk2_humps_test():' )
  print ( '  Solve the humps_ode() using rk2().' )

  t, y = rk2 ( humps_deriv, tspan, y0, n )
  ...
  plt.xlabel ( '<--- T --->' )
  plt.ylabel ( '<--- Y(T) --->' )
  plt.title ( 'humps ODE solved by rk2()' )

  filename = 'rk2_humps.png'
...
def rk2_predator_prey_test ( tspan, y0, n ):

#*****************************************************************************80
#
## rk2_predator_prey_test(): solve predator_prey_ode() by rk2().
#
#  Licensing:
#
#    This code is distributed under the MIT license.
...
#
  ...
  print ( '' )
  print ( 'rk2_predator_prey_test():' )
  print ( '  Solve the predator_prey_ode() using rk2().' )

  t, y = rk2 ( predator_prey_deriv, tspan, y0, n )
  ...
  plt.title ( 'predator prey ode solved by rk2()' )

  filename = 'rk2_predator_prey.png'
  plt.savefig ( filename )
  print ( '  Graphics saved as "%s"' % ( filename ) )
  plt.show ( block = False )
  plt.close ( )
...
def rk2_stiff_test ( tspan, y0, n ):

#*****************************************************************************80
#
## rk2_stiff_test() uses rk2() on the stiff ODE.
#
#  Discussion:
#
...
#
  ...
  print ( '' )
  print ( 'rk2_stiff_test():' )
  print ( '  Use rk2() to solve the stiff ODE.' )

  t1, y1 = rk2 ( stiff_deriv, tspan, y0, n )
  ...
  plt.xlabel ( '<-- t -->' )
  plt.ylabel ( '<-- y(t) -->' )
  plt.title ( 'stiff ode with rk2(): time plot' )
  plt.legend ( [ 'Computed', 'Exact' ] )
  filename = 'rk2_stiff.png'
  plt.savefig ( filename )
