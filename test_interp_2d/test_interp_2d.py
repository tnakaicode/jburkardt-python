...
#
  ...
  t1[0:n] = x[0:n] - 0.5
  t2[0:n] = y[0:n] - 0.5
  t3[0:n] = - 13.5 * np.exp ( - 20.25 * ( t1[0:n] * t1[0:n] + t2[0:n] * t2[0:n] ) )
  fx[0:n] = t1[0:n] * t3[0:n]
  fy[0:n] = t2[0:n] * t3[0:n]
...
#
  ...
  f[0:n] = 1.0 / ( ( 10.0 * x[0:n] - 5.0 ) ** 2 + ( 10.0 * y[0:n] - 5.0 ) ** 2 + 1.0 )
...
#
  ...
  print ( '' )
  print ( 'TEST_INTERP_2D_TEST02' )
  print ( '  Sample each function over each grid.' )
  ...
  for fi in range ( 1, f_num + 1 ):
    ...
    for gi in range ( 1, g_num + 1 ):
      ...
      f_max = max ( f[0:gn] )
      f_min = min ( f[0:gn] )
      f_ave = np.sum ( f[0:gn] ) / float ( gn )

      print ( '  %4d  %25s  %14.6g  %14.6g  %14.6g' \
...
#
  ...
  for gi in range ( 1, g_num + 1 ):
    ...
    plt.plot ( gx, gy, 'b.', markersize = 25 )
    plt.title ( gt )
    plt.grid ( True )
    plt.xlabel ( '<--- X --->' )
    plt.ylabel ( '<--- Y --->' )
