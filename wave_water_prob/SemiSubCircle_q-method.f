*   NK-problem for a Semi-Submerged Circlular Cylinder 
*                    by q-method
*
* Note : Wave number nu is stated as K in this program
*        FscI = Re{df/dz+iKf}  on inner free furface
*
      parameter ( nd=200,np1d=nd+1 )
      complex     z(np1d),zeta(np1d),zetaM(np1d)
      complex     cDcL,CPotent,CVelocity,Cpot,CVel
      real        A(np1d,np1d),RhsVec(np1d)
      real        q(np1d),qcal(np1d),Phical(np1d),Psical(np1d)
      real        LmbA,LmbI,LmbF
      real        U(np1d,np1d),V(np1d,np1d)                  
      real        W(np1d),R(np1d),ZVec(np1d),ES(np1d)
      logical     pflg/.false./
      parameter ( EpsSol=1e-5 )
      character   cond*27,CASE*1,yorn*1
      character   LCol*1
      common     /LCol/LCol
*
      LCol='y'
...
*
 100  continue
      ...
      write(6,'(a,i4)')' 50  100  200  *'
      read(5,*,err=110,end=110) n
      np1=n+1
      call BodyCdn(n,z,zeta,zetaM)
      call lcolor(3,0)
      write(6,*) '**************************************************'
      write(6,*) 'Enter y to print co-ordinates of body'
      write(6,*) '      RETURN for else'
      read(5,'(a)') yorn
      if(yorn.eq.'y') then
         call lcolor(4,0)
         call CVPRT(np1,zeta, 'BndryPnt(zeta), j=1,n+1')               
         call CVPRT(n,  z,    'Cntl-Pnt(z), i=1,n')    
         call CVPRT(n,  zetaM,'Cut-Pnt(zetaM)')  
      endif             
 10   continue
         call lcolor(7,3)
         write(6,*) '**************************************************'
         write(6,*) 'Enter Ka, Wave Number based on Radius'
         write(6,*) '            50  :  Low Speed Limit (Mirror Image)'
 ...
 30      continue
            ...
            call lcolor(7,3)
            write(6,*) '**********************************************'
            write(6,*) 'Case LmbF PsiB LmbA KyF KyA : Type of Solution'
            write(6,*) '  R     0  1/K    0   1   1 : Regular Sol.'
            write(6,*) '  Z     0    0    0   0   0 : Zero-Vertical'//
     *                                              ' Flux Sol.'
            write(6,*) '  F     0    0 -1/K   0   1 : +WQ at F.P.'
            write(6,*) '  A     0  1/K  1/K   1   0 : -WQ at A.P.'
            write(6,*) '  O     0   yF yF-yA any any: Other Any Sol.'
            write(6,*) 'Enter CASE ( R Z F A O * ) >>'
            read(5,  *  ,err=40,end=40) CASE
            call lcolor(4,0)
         if(CASE.eq.'*') goto 40
            LmbI=-cKinv
            LmbF=0
...
*
            ...
            if(icount.eq.1) then
               call CoeMat(np1d,n,A,cK,z,zeta,zetaM)
               call SOLVE (np1d,n,n,A,U,V,W,
     *                     RhsVec,R,ZVec,q,ES,pflg,epssol)
            else
...
* q and q-Calculated
            do j=1,n
               CVel=-1+CVelocity(z(j),n,zeta,q,cK,zetaM,
     *                           PsiB,LmbA,LmbI)
     ...
     *                   real(zeta(j)-zeta(j+1)))
            ...
            call lcolor(4,0)
            call vprt(n,q,   'q(j) : solutions')
            call lcolor(5,4)
            call vprt(n,qcal,'Calculated q(j) on the body')
* Phi,Psi-Calculated
            do j=1,n
               CPot=-z(j)
     *            +CPotent(z(j),n,zeta,q,cK,zetaM,PsiB,LmbA,LmbI)
               Phical(j)=real(CPot)
               Psical(j)=imag(CPot)
            enddo
            call lcolor(3,0)
...
* Values at some points on free surface
     ...
     *                 'on the free surface'
            ...
            if(yorn.eq.'y') then
               call lcolor(7,4)
               write(6,'(2a,2f8.3)') '   '//cond,' K, 1/K =',cK,1/cK
               call FreeSCond(n,zeta,q,cK,zetaM,PsiB,LmbF,LmbA,LmbI)
            endif
* cDcL=(D+iL)/((1/2)rho*U^2*(2a))
            call lcolor(3,0)
            write(6,*) 'Enter y to print Drag and Lift '//
     *                 'on the free surface'
            write(6,*) '      RETURN to skip'
            read(5,'(a)') yorn
            if(yorn.eq.'y') then
               call lcolor(7,4)
               write(6,'(2a,2f8.3)') '   '//cond,' K, 1/K =',cK,1/cK
               call cDcLcW(n,zeta,zetaM,q,PsiB,LmbA,LmbI,cK,
     *                     cDcL,cW,dR,cLI)
            endif
            goto 30
 40      continue
...
************************************************************************
     ...
     *        (n,zeta,q,cK,zetaM,PsiB,LmbF,LmbA,LmbI)
*
      real    q(*)
      real    LmbA,LmbI,LmbF
      complex zsamp,zeta(*),zetaM(*)
*
      call lcolor(5,4)
* on FA (After free surface)
      ...
      do xi=-5,-1,1
         ...
         call samplpnt(zsamp,n,zeta,q,cK,zetaM,PsiB,LmbA,LmbI)
      enddo
...
************************************************************************
      Subroutine samplpnt (z,n,zeta,q,K,zetaM,PsiB,LmbA,LmbI)
*
      real    K,q(*)
      real    LmbA,LmbI
      complex i/(0,1)/
...
*
      cf =  CPotent(z,n,zeta,q,K,zetaM,PsiB,LmbA,LmbI)
      cv =CVelocity(z,n,zeta,q,K,zetaM,PsiB,LmbA,LmbI)
      FSC=cv+i*K*cf
      write(6,'(4f8.3)') real(z),real(cv),K*imag(cf),real(FSC)
      end
************************************************************************
      Subroutine cDcLcW(n,zeta,zetaM,q,PsiB,LmbA,LmbI,cK,
     *                  cDcL,cW,dR,cLI)
*
*     cDcL=(D+iL)/((1/2)rho*U^2*(2a))
*
      real    q(*)
      real    LmbA,LmbI
      complex zeta(*),zetaM(*),zsamp
      complex cDcL,H,i/(0.0,1.0)/,eIGamma,eIzeta
      complex CPotent,CVelocity,CVel
* cDcL
      cDcL=0
      do j=1,n
         cDcL=cDcL-i*(1-q(j)**2)*conjg(zeta(j+1)-zeta(j))
      enddo
* cW, dR
      xsiF=zeta(1)
      xsiA=zeta(n+1)
      H=-2*i*((LmbI+PsiB)*exp(i*cK*xsiF)
     *      -(LmbI+PsiB-LmbA)*exp(i*cK*xsiA))
      do j=1,n
         eIGamma=(zeta(j+1)-zeta(j))/(abs(zeta(j+1)-zeta(j)))
         eIzeta=exp(i*cK*conjg(zeta(j+1)))-
     *        exp(i*cK*conjg(zeta(j)))
         H=H-2*i*q(j)*eIGamma*eIzeta/cK
      enddo
      cW=0.5*cK*abs(H)**2
      dR=cK*(PsiB**2-(PsiB-LmbA)**2)
      call lcolor(2,0)
      write(6,*) 'Pressure drag coef. D/((1/2)rho*U^2*(2a))' 
      write(6,'(a,f10.2)')  '  cD           =',real(cDcL)
      write(6,*) 'Wave resitance coef. with no correction' 
      write(6,'(a,f10.2)')  '  cW           =',cW
      write(6,*) 'Correction term for static pressure '
      write(6,'(a,f10.2)')  '  dR           =',dR
      write(6,*) 'Wave resitance coef. cW+dR must be equal to cD'
      write(6,'(a,f10.2)')  '  cW + dR      =',cW+dR
      call lcolor(6,0)
      write(6,*) 'Pressure lift coef.' 
      write(6,'(a,f10.2)')  '  cL           =',imag(cDcL)
* cLInt *********************
* cLphi
      zsamp=(zeta(n+1)+zeta(n))/2
      phiA=real(CPotent(zsamp,n,zeta,q,cK,zetaM,
     *                  PsiB,LmbA,LmbI))
      zsamp=(zeta(1)+zeta(2))/2
      phiF=real(CPotent(zsamp,n,zeta,q,cK,zetaM,
     *                  PsiB,LmbA,LmbI))
      cLphi=2*(phiF-phiA)
* cL-calculation  *
*  cLuv2F
      ...
      x3=xs
      zsamp=cmplx(x3,0)
      CVel=CVelocity(zsamp,n,zeta,q,cK,zetaM,PsiB,LmbA,LmbI)
      h3=real(CVel**2)
      cLuv2F=0
      do ix=1,NF/2
         x1=x3
         ...
         x2=x1-dx
           zsamp=cmplx(x2,0)
         h2=real(CVelocity(zsamp,n,zeta,q,cK,zetaM,PsiB,LmbA,LmbI)**2)
         x3=x2-dx
           zsamp=cmplx(x3,0)
         h3=real(CVelocity(zsamp,n,zeta,q,cK,zetaM,PsiB,LmbA,LmbI)**2)
         cLuv2F=cLuv2F+dx/3*(h1+4*h2+h3)
      enddo
*  cLuv2A
      xA=real(zeta(n+1))-0.0001
      NA=NF*(-xe-1)/(xs-1)
      x3=xA
      zsamp=cmplx(x3,0)
      CVel=CVelocity(zsamp,n,zeta,q,cK,zetaM,PsiB,LmbA,LmbI)
      ...
      do ix=1,NA/2
         ...
         x3=x2-dx
           zsamp=cmplx(x3,0)
         h3=real(CVelocity(zsamp,n,zeta,q,cK,zetaM,PsiB,LmbA,LmbI)**2)
         cLuv2A=cLuv2A+dx/3*(h1+4*h2+h3)
         cLKot=0.5*cK*imag(H**2*exp(-2*i*cK*x3))
      enddo
      eps=0.00001
      cLuv2=cLuv2F+cLuv2A
      cLInt1=cLphi+cLKot+cLuv2
 10   continue
         x1=x3
         h1=h3
         x2=x1-dx
         ...
         cLInt0=cLInt1
         cLInt1=cLphi+cLKot+cLuv2
      if(abs((cLInt1-cLInt0)/cLInt1).gt.eps) goto 10
      cLuv2=cLuv2F+cLuv2A
      cLI=cLphi+cLKot+cLuv2
      write(6,*) 'Extended theorem of Kutta-Joukowski '//
     *           'for a semi-submerged body'
      write(6,'(a,f10.2)')   '  cLdphi       =',cLphi
      write(6,'(a,f10.2,a)') '  ( cLuv2F     =',cLuv2F,' )'
      write(6,'(a,f10.2,a)') '  ( cLuv2A     =',cLuv2A,' )'
      write(6,*) 'cLuv2=cLuv2F+cLuv2A'
      write(6,'(a,f10.2)')   '  cLuv2        =',cLuv2
      write(6,'(a,f10.2)')   '  cLKotin      =',cLKot
      write(6,*) 'cLI=cLdphi+cLKotin+cLuv2 must be equal to cL'
      write(6,'(a,f10.2)')   '  cLInt        =',cLI
      call lcolor(7,2)
      write(6,*) 'Larger N is recommended if precision is not good'
      end
************************************************************************
      Subroutine CoeMat(np1d,n,A,cK,z,zeta,zetaM)
*
      real    A(np1d,*)
...
*
      ...
      do i=1,n
         ...
         RhsVec(i)=-imag(z(i))-PsiB
     *             -(LmbI+PsiB)     *imag(WQFPi)
     *             +(LmbI+PsiB-LmbA)*imag(WQAPi)
      enddo
      end
************************************************************************
      complex function CPotent(z,n,zeta,q,K,zetaM,PsiB,LmbA,LmbI)
*
*     Perturbed Complex Potential around the Body
*     
      real    K
      real    LmbA,LmbI
      complex WGint,WQfunc,WQf,WQFPi,WQAPi
      complex z,zeta(*),zetaM(*)
      real q(*)
      pi=acos(0.0)*2
      pi2Inv=1/(pi*2)
      thetacut=pi/2
*
      WQFPi=WQfunc(z,zeta(1),  K,thetacut,WQf)
      WQAPi=WQfunc(z,zeta(n+1),K,thetacut,WQf)
      CPotent=-pi2Inv*((LmbI+PsiB)*WQFPi-(LmbI+PsiB-LmbA)*WQAPi)
      do j=1,n
         CPotent=CPotent
     *         -pi2Inv*q(j)*WGint(z,zeta(j+1),zeta(j),K,zetaM(j))
      enddo
      end
************************************************************************
      complex function CVelocity(z,n,zeta,q,K,zetaM,PsiB,LmbA,LmbI)
*
*     Perturbed Complex Volocity ( u - i * v )around the Body
*     
      real    K
      real    LmbA,LmbI
      complex VGint,Wmfunc,Wmf,VQFPi,VQAPi
      complex z,zeta(*),zetaM(*)
      complex CVelF,CVelA,CVelq
      real q(*)
*
      pi=acos(0.0)*2
      pi2Inv=1/(pi*2)
*
      ...
      CVelF=-pi2Inv*(LmbI+PsiB)     *VQFPi
      CVelA= pi2Inv*(LmbI+PsiB-LmbA)*VQAPi
      CVelq=0
      do j=1,n
         CVelq=CVelq
     *         -pi2Inv*q(j)*VGint(z,zeta(j+1),zeta(j),K,zetaM(j))
      enddo
      CVelocity=CVelF+CVelA+CVelq
      end
************************************************************************
      Subroutine BodyCdn(n,z,zeta,zetaM)
*     
