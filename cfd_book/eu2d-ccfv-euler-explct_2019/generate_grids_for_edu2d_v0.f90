...
!*                    tria.grid = triangular-grid file for EDU2d solver
!*                    quad.grid = quadrilateral-grid file for EDU2d solver
!*                project.bcmap = file that contains boundary condition info
!*
!*         !Two additional files for use in EDU2D-Template code:
!*                any_name.grid = triangular-grid file for EDU2d solver
!*               any_name.bcmap = file that contains boundary condition info
!*
!*        (NOTE: The grid file format is specific to the EDU2D solver.)
!*
...
!*
!*
!*        written by Katate Masatsuka (info[at]cfdbooks.com),
!*
...
!Output - grid files
 ...
 character(80) :: datafile_tria     = "tria.grid"      !Triangular grid file for EDU2D code
 character(80) :: datafile_quad     = "quad.grid"      !      Quad grid file for EDU2D code
 character(80) :: datafile_bcmap    = "any_name.bcmap" !             BC file for EDU2D code
...
! (4)Write a boundary condition file: to be read by EDU2D code.
...
! Finally, Generate two files specific to EDU2D-Template code:

  write(*,*) "For EDU2D-Template: Writing a grid file for the triangular grid = 'any_name.grid'..."
  datafile_tria = 'any_name.grid'
  call write_grid_file(datafile_tria)

  write(*,*) "For EDU2D-Template: Generating bcmap file..."
  datafile_bcmap = 'any_name.bcmap'
  open(unit=1, file=datafile_bcmap, status="unknown", iostat=os)
  write(1,*) "Boundary Segment  Boundary Condition"
  write(1,*) "               1          dirichlet"
  write(1,*) "               2          dirichlet"
...
! Boundary data: Counterclockwise order (seeing the interior on the left)
