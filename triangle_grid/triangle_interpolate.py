...
def triangle_interpolate_linear(m, n, p1, p2, p3, p, v1, v2, v3):
    ...
    abc = triangle_area(
        np.array([[p1[0], p2[0], p3[0]], [p1[1], p2[1], p3[1]]]))

    for j in range(0, n):
        pj = np.array([p[0, j], p[1, j]])
        pbc = triangle_area(
            np.array([[pj[0], p2[0], p3[0]], [pj[1], p2[1], p3[1]]]))
        apc = triangle_area(
            np.array([[p1[0], pj[0], p3[0]], [p1[1], pj[1], p3[1]]]))
        abp = triangle_area(
            np.array([[p1[0], p2[0], pj[0]], [p1[1], p2[1], pj[1]]]))
        for i in range(0, m):
            v[i, j] =        \
                (pbc * v1[i]
                 + apc * v2[i]
